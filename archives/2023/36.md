...
updated_at: '2026-01-20 02:34:23'
labels:
  - Linux
  - '2022'
---
...
以 pnpm 为例，通过 `pnpm install` 命令安装的依赖包都将存储在 `~/Library/pnpm/store` 目录下 （7.x 版本之前全局目录是 `~/.pnpm_store`，详见 [pnpm #4522](https://github.com/pnpm/pnpm/pull/4522)） 。假设项目中安装了 jest 依赖包，会产生这样两个目录：
