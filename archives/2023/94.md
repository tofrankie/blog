...
updated_at: '2026-05-09 03:38:02'
labels:
  - Node.js
  - '2022'
---
...
如果你对 node、npm、yarn、pnpm 相关路径存疑，混乱分不清，那么这篇文章或许能帮到你。
...
使用 nvm 安装，相关内容将会存放于 `~/.nvm`。

比如，使用 `nvm install 22.22.0` 来安装 Node，它将会存放于 `~/.nvm/versions/node/v22.22.0` 目录。
...
前面提到过，`prefix` 的默认位置与 Node 的安装路径有关。`prefix` 通常是 Node 安装路径的上一级，即 `~/.nvm/versions/node/vX.Y.Z` 目录。因此切换 Node 版本时，它总能正确地安装到 `/usr/local/lib/node_modules` 或 `~/.nvm/versions/node/vX.Y.Z/lib/node_modules/`。

但如果在 `~/.npmrc` 中配置了 `prefix`，无论你如何切换 Node 版本，它总是被安装至所配置的路径下。
...
> [!NOTE]
> 可能有人注意到了，基于 nvm 的存储路径，当使用 nvm 切换 Node 时，会隔离 npm 全局依赖，但不会隔离 yarn、pnpm 的全局依赖。也就是说，在 Node 16 下安装的 npm 全局依赖，在切换到 Node 18 时不能使用。
...
目前我同时使用着 ~~nvm~~（近期重新切换回 fnm）、npm、yarn classic、pnpm，一些个人建议（仅供参考）：
...
- `~/.npm`（官网安装和 nvm 安装的 Node.js 都是该目录）
- `~/Library/Caches/Yarn/v6`
- `~/Library/pnpm/store/v10`

全局依赖目录：

- `/usr/local/lib/node_modules`（官网安装的 Node.js 的 npm 依赖目录）
- `~/.nvm/versions/node/vX.Y.Z/lib/node_modules`（使用 nvm 安装的 Node.js 的 npm 依赖目录）
- `~/Library/Application Support/fnm/node-versions/vX.Y.Z/installation/lib`（使用 fnm 安装的 Node.js 的 npm 依赖目录）
- `~/.config/yarn/global/node_modules`
- `~/Library/pnpm/global/5/node_modules`

> [!NOTE]
> 从设计上看，使用 fnm 或 nvm 切换 Node 版本时，只有 npm 全局包能做到隔离效果（npm 有 prefix 的机制），而 yarn 或 pnpm 在不同 Node 版本下安装全局依赖都会落到同一个存储目录，也就是说使用 pnpm 在 Node 22 安装的全局依赖 A，切换到 Node 20 也能直接使用全局依赖 A，起不到隔离效果，当然这种设计也是有好处的。建议尽量少地使用全局依赖，使用时应确保 Node.js 版本正确。
