...
updated_at: '2026-01-04 16:06:08'
labels:
  - 前端
  - JS
  - '2020'
---
![配图源自 Freepik](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2026/1/1767451628656.jpeg)
...
误差产生的因素有很多，比如 `for` 循环、其他异步任务（微任务、宏任务）、浏览器精度等等。本质上是 [Event Loop 机制](https://github.com/tofrankie/blog/issues/348)导致的的现象。比如示例：
...
> 
> 还有，不建议使用 `setTimeout('String Code', delay)` 的形式。因为 `eval` 通常被用来执行动态创建的代码，如果 `eval(...)` 中执行的代码包括一个或多个声明（无论变量还是函数），就会对 `eval(...)` 所处的词法作用域进行修改（可看[文章](https://github.com/tofrankie/blog/issues/268)）。避免出现一些意料之外的事情，不建议使用。
