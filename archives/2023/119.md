...
updated_at: '2025-12-25 16:59:42'
labels:
  - React
  - '2022'
---
![2022 北京冬奥会开幕式](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2025/12/1766652888203.jpeg)

此前一直在疑惑，明明 `pushState()`、`replaceState()` 不触发 `popstate` 事件，可为什么 React Router 还能挂载对应路由的组件呢？

翻了一下 [remix-run/history](https://github.com/remix-run/history) 源码，终于知道原因了。
...
`react-router-dom` 引用了 [remix-run/history](https://github.com/remix-run/history) 库 ，它主要提供了三种方法：`createBrowserHistory`、`createHashHistory`、`createMemoryHistory`。

它们用于构建对应模式的 `history` 对象（请注意，它有别于 `window.history` 对象），该对象的属性和方法可在 Devtools 中清晰地看到（如下图），也可查阅文档。这个太简单了，你们都懂，不说了。

![](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2025/12/1766652953747.png)
...
我们来看一下 [remix-run/history](https://github.com/remix-run/history) 的源码（`history/esm/history.js`）：
...
![](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2025/12/1766652988105.png)
