...
updated_at: '2025-12-06 23:48:21'
labels:
  - 前端
  - '2019'
---
...
1. 通过 CSS 修改了 Canvas 元素在文档流占据的空间，为 375 * 667
2. 通过 ctx.canvas.width = xxx 和 ctx.canvas.height = xxx 修改了 Canvas 的原始尺寸，以确保文档流占据的空间与 Canvas 原始尺寸保持正比，避免造成拉伸变形。（不理解的话，想象一下在 100 * 100 的 img 元素中放了一张 100 * 200 的图片，会造成图片展示变形）
3. 使用 CanvasRenderingContext2D.scale() 对 Canvas 的坐标系进行放大，放大了 N 倍（取决于设备 DPR），以确保后续绘制按相同的比例进行下去

为什么要这样做？

现在回归这条公式：**图片原始图片 = CSS 展示尺寸 * DPR**

> 我们在内心把 Canvas 绘制出来的图形当作一张图片。

在前面的 img 如果要在不同 DPR 设备下图片展示清晰，我们的做法是使用不同尺寸的图片。

在 Canvas 也是同理的，我们把 Canvas 本身当作一张图片。针对不同 DPR 的设备，我们不需要再换图，通过 Canvas 提供的 API 就能轻松修改其（原始）尺寸。也就是 ctx.canvas.width = xxx 和 ctx.canvas.height = xxx 所做的事情。

以 iPhone 7 为例，图片原始尺寸（ctx.canvas.width）= 375 * 2 = 750，高度同理。
...
使用 div 两层类比，它修改的是外层 div 的尺寸，此时的效果相当于：将一个 300 * 150 大小的图片放到了一个 750 * 1334 的 img 中，因此会发生变形，且在高清屏下显示不清晰。

### Canvas 绘制不清晰的原因

理解了上一节的内容，就很好理解了，并知道为什么要这样操作。

在高清屏下，Canvas 绘制模糊的原因是没有进行以下两部操作：
...
```js
// 仅确保了 Canvas 的实际尺寸跟文档流占据的空间一致，以避免变形
ctx.canvas.width = elementWidth
ctx.canvas.height = elementHeight
```
