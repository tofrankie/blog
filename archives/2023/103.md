...
updated_at: '2026-07-09 03:33:00'
labels:
  - Git
  - 持续更新
  - '2020'
...
![](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2026/1/1767417455234.png)

![](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2026/1/1767417470934.png)
...
有时，当远程上某个分支已经被删除（比如合并后删了），本地可能会残留已不存在于远程的 `origin/xxx` 引用，导致 `git branch -r` 列表越来越脏。

可以通过以下命令自动清除过时的远程跟踪分支：
...
![](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2026/1/1767417832635.png)
...
## 找回 git reset --hard 的代码

> 关于 `git reset` 的三种模式介绍，推荐这篇[文章](https://www.jianshu.com/p/c2ec5f06cf1a)。

假设误执行了 `git reset --hard HEAD^`（移动 HEAD 至当前 commit 的上一个 commit，并舍弃变更）导致代码丢失。

解决方法：

1. 通过 `git reflog` 找到对应的 commit-id
2. 执行 `git reset --hard <commit-id>` 重新回到对应的记录。

> 当我们执行 `git reset` 或 `git cherry-pick` 时，Git 会把这些操作记录在**本地**，这些操作记录可以通过 `git reflog` 来找到。
