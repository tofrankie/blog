...
updated_at: '2026-07-21 02:47:05'
labels:
  - Mac
  - Homebrew
  - '2017'
...
相比 3.0 有一个较大的变化，默认使用从 [Formulae.brew.sh](https://formulae.brew.sh/) 下载的 JSON 文件进行包的安装，而不是本地 `homebrew/core` 和 `homebrew/cask` 仓库源。

检查清单：

- 如果你不是 Homebrew 包开发者，可以分别执行命令 `brew untap homebrew/core` 和 `brew untap homebrew/cask` 来节省一些空间。
