...
updated_at: '2026-01-02 03:36:17'
labels:
  - Node.js
  - '2023'
---
![配图源自 Freepik](https://cdn.jsdelivr.net/gh/tofrankie/blog@main/images/2026/1/1767295952503.jpg)
...
由于 `yarn install` 或 `npm install` 走的是 HTTPS 协议，它的安全通过数字证书来保障。数字证书由专门机构颁发，通常是付费的。自签证书，就是自己扮演数字证书机构给自己颁发的证书。
...
而 `https://r2.cnpmjs.org/form-data/-/form-data-3.0.1.tgz` 所在域名的证书是不被信任的。通过 Firefox 浏览器访问可以看到警告：
...
在**确保安全**的情况下，可以临时关闭 `strict-ssl` 选项：
