...
updated_at: '2026-03-06 04:51:28'
labels:
  - 前端
  - '2022'
---
...
如果消除掉重复部分，那么整个文件占用空间将会大幅降低，这种方式称为「透明度优化」。

借助 Gifsicle 工具可实现透明度优化，命令如下：
...
是的，这个才是 GIF 图的最初模样。原图大小为 2.1MB，透明度处理之后大小为 250KB，因此透明度优化的收益是巨大的。

请注意，经透明度优化处理后，显示效果或许会受到 Disposal Method（定义帧之间的叠加效果）的影响，但一般情况下无需特别设置。若有需要 Gifsicle 也提供了 `--disposal` 参数供处理。
...
当然，并不是所有图片如第一组般处理时，效果都那么差。但批量处理图片的话，应先还原透明度，再抽帧，再进行透明度优化。
...
目前已编写了一个 Shell 脚本进行批量“无损”地处理 GIF 图片，主要优化点是透明度优化、移除本地颜色表、移除对渲染无用的扩展内容。
...
详见：[tofrankie/gif-optimize](https://github.com/tofrankie/gif-optimize)
