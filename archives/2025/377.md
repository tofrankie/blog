...
updated_at: '2025-11-24 18:32:41'
labels:
  - '2022'
  - 前端
---
...
也可以尝试 Gifsicle 提供的 --lossy 有损压缩选项，具体细节其文档中未体现（有兴趣可看 [PR #16](https://github.com/kohler/gifsicle/pull/16)），因此实际效果不可预测。
...
添加帧：打开两组图片，选择上述模式之一，然后两组图片之间进行拖拽即可。比如，从 A 中拖拽一帧到 B，A 帧数不会发生变化，B 则会增加一帧。
...
2. 抽取指定帧：传入指定帧数，形如如 #3 #9。但这种方式不适用于批量处理。
