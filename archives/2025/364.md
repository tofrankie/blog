...
updated_at: '2026-09-19 20:40:04'
labels:
  - '2025'
  - Tools
---
...
是的，我已经将 [Alfred](https://www.alfredapp.com/) 换为 [Raycast](https://raycast.com/?via=tofrankie) 了，后者的免费功能足够好用。
...
> [Raycast vs Alfred](https://www.raycast.com/raycast-vs-alfred?via=tofrankie)

最重要的是，Raycast Extension 的开发体验比 Alfred 好太多了。官方提供了很多便捷好用的 API、统一的 UI 风格、有官方分发平台 [Raycast Store](https://www.raycast.com/store?via=tofrankie)、[活跃度](https://github.com/raycast/extensions/pulse)很高、目前扩展数量 [2200+](https://github.com/raycast/extensions/blob/cffac925444c27172ccf9538a15bad35a97a0adf/.github/CODEOWNERS)。
...
虽然使用 React 构建，但只能使用官方提供的内置 UI 组件，不能过多地自定义（可能是 React to macOS Native 的缘故吧），好处是统一的 UI 体验。
...
> macOS 扩展路径：`~/Library/Application Support/com.raycast.macos/extensions`

> Windows 扩展路径：`%LocalAppData%\Raycast\extensions\Raycast\extensions`

### 创建扩展

键入 Create Extension 回车填写必要字段便创建好了。
...
提交 PR 时，建议附上视频/截图以便于审核。我之前写了一个微信开发工具的[扩展](https://github.com/tofrankie/raycast-wechat-devtool)，由于他们没有小程序项目，流程上它们跑不通，可能会要求附上视频辅助审核，例如[这个](https://github.com/raycast/extensions/pull/20222#pullrequestreview-3003917464)。
...
- **platforms**：现在 Raycast 支持 macOS 和 Windows 双平台。在开发跨平台的扩展时，避免使用某个操作系统特有的 API 或快捷键等。在 Node.js 中可以用 `process.platform === 'darwin'` 和 `process.platform === 'win32'` 来判断操作系统
...
常用快捷键内置了一些常量（[更多](https://developers.raycast.com/api-reference/keyboard#keyboard.shortcut.common)），它们是跨平台的，比如：
...
若你的扩展仅支持 macOS 时，可以如下设置（Windows 同理）：
...
以后者为例，可以将项目名称、项目路径、项目名称拼音（若有中文）添加到 keywords 字段，参考 [pinyin.ts](https://github.com/tofrankie/raycast-collection/blob/812004f75301a8cf1dd52dbca2c9446272193df2/extensions/wechat-devtool/src/utils/pinyin.ts)。
...
扩展截图在 Raycast Store 展示用，存放在 `metadata` 目录下，可提供 1 ~ 6 张截图。文件名未作限制，建议 `screenshot-1.png`、`<extension-name>-1.png` 这类形式。

Raycast 内置了截屏功能，在 Raycast Settings - Advanced - Window Capture 中设置快捷键（如 ⌥ + ⇧ + ⌘ + M）。

在开发模式下（使用 `npm run dev`）启动扩展，按下快捷键便可调出截屏窗口，勾选「Save to Metadata」，截图时自动保存到项目的 `metadata` 目录（不行的话移进去），更多请看[这里](https://developers.raycast.com/basics/prepare-an-extension-for-store#screenshots)。

> Raycast 提供了一些高分辨的[高清壁纸](https://www.raycast.com/wallpapers?via=tofrankie)

## 参与贡献

如果想对已发布的扩展作出贡献：

1. 如果你的变更很大，最好在投入大量时间之前联系扩展的作者。未经作者同意，可能无法完成合并
2. fork [raycast/extensions](https://github.com/raycast/extensions)
3. 功能调整...
4. 将你的 Raycast 用户名添加到 package.json 的 `contributors` 字段
5. 提交 PR，等待审核发布

由于审核较慢，如果同时多个扩展做出贡献，建议使用不同的分支提交 PR，如 `ext/extension-a`、`ext/extension-b`。
