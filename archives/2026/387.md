...
updated_at: '2026-02-05 05:18:03'
labels:
  - '2026'
  - AI
---
...
根据官方[要求](https://support.google.com/gemini/answer/16283624)，满足以下条件方可使用：
...
现在，Chrome App 的语言设置通常会根据系统语言来设定。
...
1. 修改前备份 `Local State` 到 `Local State.bak`
2. 修改 `is_glic_eligible`、`variations_country`、`variations_permanent_consistency_country` 选项

## 最后

在 `chrome://settings/ai/gemini` 可以为 Gemini in Chrome 调整配置。

另外，Gemini in Chrome 和原有的 Side Panel 分居左右两侧，我更喜欢将 Gemini in Chrome 的 Chat Panel 放在网页右侧，此时可以将 `chrome://settings/appearance` 的 `Side panel position` 改为 `Show on left`。
